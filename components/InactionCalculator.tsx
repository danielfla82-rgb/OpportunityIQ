...
import { Snowflake, Flame, ArrowRight, Loader2, AlertCircle } from 'lucide-react';
      ...
      <div className="lg:col-span-2 space-y-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            O Viés do Status Quo faz você acreditar que não fazer nada é grátis. Não é. Calcule o "Custo da Covardia".
          </p>
        </div>
        ...
        <div className="bg-slate-900/50 p-6 rounded-xl border border-slate-800 flex flex-col gap-4">
          ...
          <div>
             <label className="block text-xs text-slate-500 uppercase tracking-widest mb-2">Custo Financeiro Mensal de Esperar (R$)</label>
             <input 
                type="number"
                className="w-full bg-slate-950 border border-slate-700 rounded p-3 text-slate-200 outline-none focus:border-cyan-500"
                placeholder="Perda de renda, custo de manutenção, taxas..."
                value={financialCost}
          ...
          <div>
             <label className="block text-xs text-slate-500 uppercase tracking-widest mb-2">Custo Emocional (Nível de Estresse 1-10)</label>
             <input 
                type="range"
                min="0" max="10"
             ...
             />
             <div className="text-right text-cyan-400 text-sm mt-1 font-mono">Nível {emotionalCost}</div>
             <p className="text-[10px] text-slate-500 mt-1">
               Calculamos isso como horas de "largura de banda mental" consumidas da sua THL.
             </p>
          </div>
         ...
         {!analysis && !loading && (
            <div className="h-full flex flex-col items-center justify-center border-2 border-dashed border-slate-800 rounded-xl bg-slate-900/20 p-8 text-center min-h-[400px]">
              ...
              <p className="text-slate-500 max-w-xs">
                A inércia é uma força poderosa. Quebre-a visualizando o custo acumulado.
              </p>
            </div>
         )}
